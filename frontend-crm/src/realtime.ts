...
export function connectRealtime() {
  try {
    ...
    socket = io(SOCKET_URL, {
      ...
      /**
       * Через прокси Vercel — ТОЛЬКО опрос (polling).
       *
       * Переписывания в vercel.json проксируют обычный HTTP, но НЕ выполняют
       * протокольный апгрейд до WebSocket: попытка апгрейда возвращает 400.
       * Socket.IO после этого молча откатывается на опрос и работает —
       * но каждая страница у каждого сотрудника писала в консоль
       * «WebSocket handshake failed», и настоящая ошибка потонула бы в этом
       * шуме. Проверено браузерным прогоном: 40 одинаковых записей на
       * 17 разделов × 4 роли.
       *
       * Держать 'websocket' в списке «на всякий случай» смысла нет: сокет
       * через этот адрес не поднимется никогда, пока фронт живёт за
       * переписыванием. Опрос для наших объёмов достаточен — в сокет уходят
       * только идентификаторы, содержимое фронт забирает по HTTP.
       *
       * В разработке адрес прямой (localhost:3001), апгрейд проходит —
       * там оставляем websocket первым.
       */
      transports: isAbsoluteApi ? ['websocket', 'polling'] : ['polling'],
      reconnection: true,
