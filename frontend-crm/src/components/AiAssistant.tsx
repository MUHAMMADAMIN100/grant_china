...
export default function AiAssistant() {
  ...
  const onAsk = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Предупреждения об охвате: сотрудник должен понимать, на чём построен
      // ответ, особенно когда помощник считает («у скольких нет визы») — иначе
      // неполный подсчёт выглядит как точный.
      if (res.articles === 0 && res.students === 0) {
        setNotice('Помощнику не на что опираться: база знаний пуста и доступных студентов нет.');
      } else if (res.truncated) {
        setNotice('Данные не поместились в ответ целиком — часть могла быть не учтена.');
      } else if (res.articles === 0) {
        setNotice('База знаний пуста — по регламентам помощник ответить не сможет. Наполните раздел «База знаний».');
      } else {
        setNotice(null);
      }
    } catch (err: any) {
