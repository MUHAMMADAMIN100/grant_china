...
/**
 * Потолок внутреннего размышления модели.
 *
 * Прямое следствие подъёма MAX_OUTPUT_TOKENS. Gemini 3 Pro тратит бюджет вывода
 * и на рассуждения тоже, поэтому «разрешили ответ подлиннее» на практике
 * означало «разрешили думать вчетверо дольше»: на боевом объёме (204 студента
 * плюс база знаний) ответ переставал укладываться в TIMEOUT_SEC, хотя по
 * содержанию это одна строка. Замер на копии боевой базы: 4096 — ответ за
 * 33–35 секунд, 16 000 без потолка размышления — таймаут дважды подряд.
 *
 * Ограничивается именно размышление, а не ответ: перечисление студентов должно
 * помещаться целиком, ради этого лимит вывода и поднимали.
 *
 * Значение подобрано замерами по реальному пути приложения на копии боевой базы
 * (204 студента, 17 статей, ~24 000 токенов входа), три разных вопроса:
 *   без потолка — таймаут, ответ приходил от резервной модели;
 *   4096        — часть вопросов всё ещё не укладывалась в 55 секунд;
 *   1024        — 16, 43 и 25 секунд, все ответы от основной модели.
 *
 * 43 секунды из этого ряда — запас небольшой, поэтому переход на резервную
 * модель по таймауту оставлен как страховка: в редкий неудачный раз сотрудник
 * получит ответ от flash, а не ошибку.
 */
const THINKING_BUDGET = Number(process.env.AI_THINKING_BUDGET) || 1024;
...
export class AiService {
  ...
  async ask(question: string, user: CurrentUser): Promise<AiAskResult> {
    ...
    try {
      ...
      //
      // ТАЙМАУТ (504) СЮДА ЖЕ. Для сотрудника «модель перегружена» и «модель
      // думает дольше, чем мы готовы ждать» — одно и то же: ответа нет. Резервная
      // модель быстрее основной вдвое и на том же вопросе укладывается (33 с
      // против таймаута — замер на копии боевой базы), так что молчать и отдавать
      // ошибку, имея под рукой работающую модель, здесь тем более незачем.
      const overloaded =
        e instanceof GeminiError && (e.status === 503 || e.status === 429 || e.status === 504);
      if (overloaded && this.model !== FALLBACK_MODEL) {
        this.logger.warn(
          `AI: модель «${this.model}» не ответила (${(e as GeminiError).status}), переключаюсь на «${FALLBACK_MODEL}»`,
        );
        try {
