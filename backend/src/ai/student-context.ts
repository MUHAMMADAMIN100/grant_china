 ...
 *
 * Было 200 — и при 203 студентах в базе трое не доходили до помощника, а
 * сотрудник видел плашку «данные не поместились» на каждом вопросе. Ради
 * ровного числа терялись реальные люди, поэтому потолок поднят до 5000.
 *
 * Это НЕ «сколько влезет», а страховка от неограниченного запроса. Одна строка
 * студента — примерно 120 токенов, то есть 5000 студентов дают порядка 600
 * тысяч токенов: столько уже не поместится в окно модели, и оборвётся всё
 * равно, но по вине Google, а не по нашему решению. Число выбрано так, чтобы
 * до него агентство росло годами, а признак усечения оставался рабочим
 * механизмом, а не декорацией.
 *
 * Если база подойдёт к этому пределу, правильный ответ — не поднимать число
 * дальше, а научить помощника запрашивать выборку под вопрос, а не всю базу
 * на каждый вопрос.
 */
