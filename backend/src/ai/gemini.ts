...
export async function callGemini(req: GeminiRequest): Promise<GeminiResult> {
  ...
  // 55 секунд, а не «побольше на всякий случай». Запрос от браузера идёт не
  // напрямую в Railway, а через rewrite на Vercel, и у того есть собственный
  // потолок ожидания около минуты. Если ждать дольше проксирующего слоя, он
  // оборвёт соединение первым — и сотрудник увидит невнятную сетевую ошибку
  // вместо нашего «помощник не ответил вовремя, попробуйте ещё раз».
  //
  // Замеры на живом ключе: Pro отвечает за 8–18 секунд на коротком вопросе.
  // С полным контекстом (база знаний + до 200 студентов) будет дольше, поэтому
  // запас нужен — но не за пределами того, что доживёт до браузера.
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), (req.timeoutSec ?? 55) * 1000);
