...
export class ApplicationsService implements OnModuleInit {
  ...
  /**
   * Тексты SMS студенту по этапам — дословно от владельца (12.08.2026).
   *
   * ПРО ЦЕНУ: кириллица в SMS кодируется UCS-2, то есть 70 символов на одно
   * сообщение (не 160, как латиница). Эти тексты — 130–210 символов, значит
   * каждое уходит как 2–4 SMS по тарифу. Владелец выбрал отправлять их
   * дословно, зная это. Сокращать «по-тихому» нельзя: тексты — его слова,
   * и правка сделала бы отправляемое отличным от согласованного.
   *
   * NEW сюда включён намеренно: заявка может быть возвращена на «Новая»
   * откатом, и тогда сработает ветка isDowngrade ниже, а не этот текст.
   * IN_PROGRESS/COMPLETED — исторические значения (мигрированы в DOCS_REVIEW
   * и ENROLLED), новых переходов в них нет; на всякий случай ведут себя как
   * их современные пары.
   */
  private static STAGE_SMS: Partial<Record<ApplicationStatus, string>> = {
    NEW:
      'Ваша заявка успешно создана! 🎉\n' +
      'Мы получили ваши данные и начинаем подготовку документов. Следующий этап — проверка документов.',
    DOCS_REVIEW:
      'Ваши документы находятся на проверке 🔍\n' +
      'Мы внимательно проверяем все данные перед подачей в университет. Если всё в порядке, заявка перейдёт на следующий этап.',
    DOCS_SUBMITTED:
      'Отличные новости! 🎉\n' +
      'Ваши документы успешно поданы в университет. Теперь ожидаем рассмотрения вашей заявки и решения университета. Желаем удачи! 🍀',
    PRE_ADMISSION:
      'Поздравляем! 🎉 Вы получили зачисление в университет!\n' +
      'Теперь мы подготовим для вас дальнейшие инструкции по оплате и оформлению документов.',
    AWAITING_PAYMENT:
      'Ваша заявка перешла на этап оплаты 💳\n' +
      'Пожалуйста, ознакомьтесь с информацией об оплате и внесите необходимую сумму в указанный срок.',
    ENROLLED:
      'ПОЗДРАВЛЯЕМ! 🎉🎓\n' +
      'Вы официально зачислены в университет!\n' +
      'Ваша заявка успешно завершена. Добро пожаловать в Китай! 🇨🇳\n' +
      'Мы остаёмся с вами и поможем на следующих этапах — от подготовки к поездке до приезда в Китай.',
    // Исторические значения — ведут себя как современные пары.
  ...
  private smsTextForStatus(prev: ApplicationStatus, next: ApplicationStatus): string {
    ...
    // ОТКАТ НАЗАД сохраняет прежний нейтральный текст. Слать «Поздравляем,
    // вы зачислены» человеку, которого только что вернули на проверку
    // документов, — прямая ложь; а «документы поданы» при откате с оплаты
    // заставит его думать, что всё идёт по плану.
    ...
    // Исторические статусы приводим к современным парам.
    const key: ApplicationStatus =
      next === 'IN_PROGRESS' ? 'DOCS_REVIEW' : next === 'COMPLETED' ? 'ENROLLED' : next;
    const staged = ApplicationsService.STAGE_SMS[key];
