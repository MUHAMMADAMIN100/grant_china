...
export class StudentsService implements OnModuleInit {
  ...
  private hasAccess(
    ...
    // ОТКАТ ОГРАНИЧЕНИЯ ПЕРЕДАЧИ (решение владельца, 12.08.2026): здесь стояло
    // «китайскому менеджеру карточка открывается только после передачи»
    // (transferredToChinaAt === null → false). Убрано вместе с парным условием
    // в фильтре findAll — менеджер снова видит студента, как только вписан в
    // chinaManagerId. Подробное «почему» — у фильтра списка. Если ограничение
    // вернут, оно обязано появиться в обоих местах одновременно.
  ...
  async findAll(filters: {
    ...
    if (restrictToMine) {
      ...
      // ОТКАТ ОГРАНИЧЕНИЯ ПЕРЕДАЧИ (решение владельца, 12.08.2026).
      // Первая редакция ТЗ «Разделение воронок» скрывала от китайского
      // менеджера студентов до нажатия «Передать в Китай»:
      //   if (region === 'CN') and.push({ transferredToChinaAt: { not: null } })
      // На практике это оставило китайский офис с пустыми списками: у 198
      // студентов из 203 китайский менеджер вписан заранее, «на будущее», и
      // все они спрятались разом. Владелец решил вернуть прежнюю видимость:
      // менеджер видит студента, как только вписан в chinaManagerId.
      //
      // Граница офисов при этом живёт в ФИНАНСАХ, а не в карточках:
      // payment-rules.ts (STAGE_OFFICE/stagesForUser) по-прежнему скрывает от
      // CN таджикские этапы и суммы — и в сводке, и в графике, и по прямой
      // ссылке. Передача осталась отметкой момента «студент ушёл из
      // Таджикистана»: дата, уведомление принимающему, запись в Активности.
      //
      // Если ограничение когда-нибудь вернут — ставить его надо В ОБА места
      // сразу: сюда и в hasAccess(). Пока условие жило только здесь, студент
      // не показывался в списке, но карточка открывалась по прямой ссылке —
      // третья такая асимметрия в проекте (регионы, платежи, теперь это).
    }
